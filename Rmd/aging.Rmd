...
**Passo 1** - Importar a base de dados.  Os dados estão contidos em um arquivo de texto de formato fixo como pode ser visto no extrato acima. Para importar arquivos neste formato, o R dispõe da função `read.fwf()`.

```{r}
# Define o diretório onde está o arquivo
setwd("C:\\Users\\Marcos\\Documents\\GitHub\\Usando-R\\dados")

# Leitura do arquivo
ctsRec <- read.fwf("Arfile.txt", widths=c(11, 4, 4, 15, 8))
...
perfil <- transform(perfil, TOTAL.PERCT  = round(TOTAL / sum(TOTAL) * 100, digits=4))
perfil
```
